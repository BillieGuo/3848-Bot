...
bool Front_flag = false, Left_flag = false, Right_flag = false, Back_flag = false;
int obstacle_cnt = 0, front_cnt = 0, left_cnt = 0, right_cnt = 0;
int last_avoid_move = 0;
...
//     Sonar_distance_in_cm = (Sonar_duration/0.6) / 29.1;
//     done = true;
//   }
// }
    ...
    case 0b00000: //no obstacle
      if (Front_flag){
        if (Left_flag && Right_flag){
          Move(0.0, -0.6, 0.0); // if both block, move backward
        }
        else if (Left_flag){
          Move(0.6, 0.0, 0.0); // if left block, move right
        }
        else if (Right_flag){
          Move(-0.6, 0.0, 0.0); // if right block, move left
        }
        else {
          Move(-0.6, 0.0, 0.0);
        }
        ...
        Move(0.0, 0.6, 0.0); // if no block, move forward
      }
      break;
...
void Line_tracking(){
  ...
  Serial.print("Grayscale_combined: ");
  Serial.println(Grayscale_combined);
  switch (Grayscale_combined){
    case 0b00000: // no white line detected
      if (last_avoid_move == -1){ // go inversely to the last avoid move
        Move(0.6, 0.0, 0.0);
      }
      else if (last_avoid_move == 1){
        Move(-0.6, 0.0, 0.0);
      }
      else {
        Move(0.0, 0.6, 0.0);
    ...
    case 0b00110: // middle and middle right detect white, may be court edge, go straight
    case 0b00111: // middle, middle right and right most detect white, may be court edge, straight
    ...
    case 0b01100: // middle left detect white, may be court edge, go straight
    ...
    case 0b10111: // left most, middle, middle right and right detect white, may be court edge, go straight
    case 0b11100: // left most, middle left and middle detect white, may be court edge, go straight
    case 0b11101: // left most, middle left, middle and right most detect white, may be court edge, go straight
